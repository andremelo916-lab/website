...
class Geometry {
  ...
  subdivide(divisions = 1) {
    ...
    for (let div = 0; div < divisions; ++div) {
      ...
      f.forEach((face, ndx) => {
        ...
        const i = ndx * 4
        newFaces[i+0] = new Face(face.a, mAB, mCA); newFaces[i+1] = new Face(face.b, mBC, mAB)
        newFaces[i+2] = new Face(face.c, mCA, mBC); newFaces[i+3] = new Face(mAB, mBC, mCA)
      })
      f = newFaces
    }
  ...
  getMidPoint(ndxA, ndxB, cache) {
    ...
    const a = this.vertices[ndxA].position; const b = this.vertices[ndxB].position
    const ndx = this.vertices.length; cache[cacheKey] = ndx
    this.addVertex((a[0]+b[0])*0.5, (a[1]+b[1])*0.5, (a[2]+b[2])*0.5); return ndx
  }
}
...
function createProgram(gl, shaderSources, transformFeedbackVaryings, attribLocations) {
  ...
  ;[gl.VERTEX_SHADER, gl.FRAGMENT_SHADER].forEach((type, ndx) => {
    const shader = createShader(gl, type, shaderSources[ndx]); if (shader) gl.attachShader(program, shader)
  })
...
function makeVertexArray(gl, bufLocNumElmPairs, indices) {
  ...
  for (const [buffer, loc, numElem] of bufLocNumElmPairs) {
    if (loc === -1) continue
    gl.bindBuffer(gl.ARRAY_BUFFER, buffer); gl.enableVertexAttribArray(loc); gl.vertexAttribPointer(loc, numElem, gl.FLOAT, false, 0, 0)
  }
...
function resizeCanvasToDisplaySize(canvas) {
  ...
  const dw = Math.round(canvas.clientWidth * dpr); const dh = Math.round(canvas.clientHeight * dpr)
...
function makeBuffer(gl, sizeOrData, usage) {
  const buf = gl.createBuffer(); gl.bindBuffer(gl.ARRAY_BUFFER, buf); gl.bufferData(gl.ARRAY_BUFFER, sizeOrData, usage); gl.bindBuffer(gl.ARRAY_BUFFER, null); return buf
}
...
class ArcballControl {
  ...
  update(deltaTime, targetFrameDuration = 16) {
    const timeScale = deltaTime / targetFrameDuration + 0.00001; let angleFactor = timeScale; let snapRotation = quat.create()
    if (this.isPointerDown) {
      ...
      if (vec2.sqrLen(midPointerPos) > this.EPSILON) {
        ...
        vec2.copy(this.previousPointerPos, midPointerPos); angleFactor *= ANGLE_AMPLIFICATION
        this.quatFromVectors(a, b, this.pointerRotation, angleFactor)
      } else { quat.slerp(this.pointerRotation, this.pointerRotation, this.IDENTITY_QUAT, INTENSITY) }
    } else {
      const INTENSITY = 0.1 * timeScale; quat.slerp(this.pointerRotation, this.pointerRotation, this.IDENTITY_QUAT, INTENSITY)
      if (this.snapTargetDirection) {
        const SNAPPING_INTENSITY = 0.2; const a = this.snapTargetDirection; const b = this.snapDirection
        const sqrDist = vec3.squaredDistance(a, b); const distanceFactor = Math.max(0.1, 1 - sqrDist * 10)
        angleFactor *= SNAPPING_INTENSITY * distanceFactor; this.quatFromVectors(a, b, snapRotation, angleFactor)
      }
    }
    ...
    const RA_INTENSITY = 0.8 * timeScale; quat.slerp(this._combinedQuat, this._combinedQuat, combinedQuat, RA_INTENSITY); quat.normalize(this._combinedQuat, this._combinedQuat)
    const rad = Math.acos(this._combinedQuat[3]) * 2.0; const s = Math.sin(rad / 2.0); let rv = 0
    ...
    const RV_INTENSITY = 0.5 * timeScale; this._rotationVelocity += (rv - this._rotationVelocity) * RV_INTENSITY; this.rotationVelocity = this._rotationVelocity / timeScale
  ...
  #project(pos) {
    ...
    const x = (2*pos[0] - w - 1) / s; const y = (2*pos[1] - h - 1) / s; let z = 0; const xySq = x*x + y*y; const rSq = r*r
...
class InfiniteGridMenu {
  ...
  run(time = 0) {
    this.#deltaTime = Math.min(32, time - this.#time); this.#time = time; this.#deltaFrames = this.#deltaTime / this.TARGET_FRAME_DURATION; this.#frames += this.#deltaFrames
  ...
  #init(onInit) {
    this.gl = this.canvas.getContext('webgl2', { antialias: true, alpha: true }); const gl = this.gl
  ...
  #initTexture() {
    ...
    Promise.all(this.items.map(item => new Promise(resolve => {
      const img = new Image(); img.crossOrigin = 'anonymous'; img.onload = () => resolve(img); img.onerror = () => resolve(null); img.src = item.image
    }))).then(images => {
      images.forEach((img, i) => {
        if (!img) return; const x = (i % this.atlasSize) * cellSize; const y = Math.floor(i / this.atlasSize) * cellSize
  ...
  #initDiscInstances(count) {
    ...
    for (let i = 0; i < count; ++i) {
      const arr = new Float32Array(this.discInstances.matricesArray.buffer, i * 16 * 4, 16); arr.set(mat4.create()); this.discInstances.matrices.push(arr)
    }
    ...
    for (let j = 0; j < 4; ++j) {
      const loc = this.discLocations.aInstanceMatrix + j; gl.enableVertexAttribArray(loc)
      gl.vertexAttribPointer(loc, 4, gl.FLOAT, false, bytesPerMatrix, j * 4 * 4); gl.vertexAttribDivisor(loc, 1)
    }
  ...
  #updateProjectionMatrix(gl) {
    this.camera.aspect = gl.canvas.clientWidth / gl.canvas.clientHeight; const height = this.SPHERE_RADIUS * 0.35; const distance = this.camera.position[2]
    if (this.camera.aspect > 1) { this.camera.fov = 2 * Math.atan(height / distance) } else { this.camera.fov = 2 * Math.atan(height / this.camera.aspect / distance) }
    mat4.perspective(this.camera.matrices.projection, this.camera.fov, this.camera.aspect, this.camera.near, this.camera.far)
    mat4.invert(this.camera.matrices.inversProjection, this.camera.matrices.projection)
  }

  #onControlUpdate(deltaTime) {
    const timeScale = deltaTime / this.TARGET_FRAME_DURATION + 0.0001; let damping = 5 / timeScale; let cameraTargetZ = 3 * this.scaleFactor
    const isMoving = this.control.isPointerDown || Math.abs(this.smoothRotationVelocity) > 0.01
    if (isMoving !== this.movementActive) { this.movementActive = isMoving; this.onMovementChange(isMoving) }
    if (!this.control.isPointerDown) {
      const nearestVertexIndex = this.#findNearestVertexIndex(); const itemIndex = nearestVertexIndex % Math.max(1, this.items.length)
      this.onActiveItemChange(itemIndex); const snapDirection = vec3.normalize(vec3.create(), this.#getVertexWorldPosition(nearestVertexIndex))
      this.control.snapTargetDirection = snapDirection
    } else { cameraTargetZ += this.control.rotationVelocity * 80 + 2.5; damping = 7 / timeScale }
    this.camera.position[2] += (cameraTargetZ - this.camera.position[2]) / damping; this.#updateCameraMatrix()
  }

  #findNearestVertexIndex() {
    const n = this.control.snapDirection; const inversOrientation = quat.conjugate(quat.create(), this.control.orientation)
    const nt = vec3.transformQuat(vec3.create(), n, inversOrientation); let maxD = -1; let nearestVertexIndex
...
export default function InfiniteMenu({ items = [], scale = 1.0, onItemClick }) {
  ...
  useEffect(() => {
    ...
    const sketch = new InfiniteGridMenu(
      canvas,
      items.length ? items : [{ image: 'https://picsum.photos/300/300?grayscale', link: '#', title: '', description: '' }],
      index => {
    ...
    return () => { window.removeEventListener('resize', handleResize); sketchRef.current?.destroy() }
  }, [items, scale])

  const handleButtonClick = () => {
    if (onItemClick) { onItemClick(activeIdx); return }
    if (!activeItem?.link) return
    if (activeItem.link.startsWith('http')) { window.open(activeItem.link, '_blank') }
  }

  // Detect tap on canvas (pointer down → up with < 8px movement) → open active item
  const pointerDownPos = useRef(null)
  const handleCanvasPointerDown = (e) => {
    pointerDownPos.current = { x: e.clientX, y: e.clientY }
  }
  const handleCanvasPointerUp = (e) => {
    if (!pointerDownPos.current || !activeItem) return
    const dx = e.clientX - pointerDownPos.current.x
    const dy = e.clientY - pointerDownPos.current.y
    if (Math.sqrt(dx * dx + dy * dy) < 8) handleButtonClick()
